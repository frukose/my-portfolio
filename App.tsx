...
const App: React.FC = () => {
    ...
    <div className="animate-in fade-in slide-in-from-right-8 duration-700 py-16 px-6 max-w-6xl mx-auto">
      ...
      <div className="grid lg:grid-cols-3 gap-16">
        ...
        <div className="relative">
          <img src="https://images.unsplash.com/photo-1555066931-4365d14bab8c?auto=format&fit=crop&q=80&w=800" className="rounded-[3rem] shadow-2xl grayscale" alt="Tech" />
        </div>
      </div>
    </div>
  );
  ...
  const renderSkills = () => (
    <div className="animate-in fade-in slide-in-from-left-8 duration-700 py-16 px-6 max-w-7xl mx-auto">
      <h2 className="text-5xl md:text-7xl font-black mb-12 tracking-tight text-center">The <span className="text-indigo-600">Toolkit.</span></h2>
  ...
  const renderProjects = () => (
    <div className="animate-in fade-in zoom-in-95 duration-700 py-16 px-6 max-w-7xl mx-auto">
      <h2 className="text-5xl md:text-7xl font-black mb-12 tracking-tight">Recent <span className="text-indigo-600">Work.</span></h2>
      <div className="grid md:grid-cols-3 gap-10">
        {(isLoadingProjects ? Array(3).fill(null) : projects).map((project, i) => (
          project ? (
            <div key={project.id} className="bg-white rounded-[3rem] overflow-hidden shadow-sm border border-gray-100 flex flex-col h-full">
              <img src={project.imageUrl} alt={project.name} className="h-64 object-cover" />
              <div className="p-10 flex flex-col flex-1">
                ...
                <button onClick={() => setSelectedProject(project)} className="mt-auto w-full py-5 bg-gray-900 text-white rounded-2xl font-black text-sm">View Details</button>
              </div>
            </div>
          ) : (
      ...
      <div className="bg-gray-900 rounded-[4rem] p-10 md:p-20 text-white grid lg:grid-cols-2 gap-20 overflow-hidden relative shadow-3xl">
        <div className="relative z-10">
          ...
          <div className="space-y-10">
            <div className="flex items-center gap-8">
              ...
              <a href="https://wa.me/2347030195046" target="_blank" className="text-2xl font-black">07030195046</a>
            </div>
            <div className="flex items-center gap-8">
        ...
        <div className="bg-white rounded-[3rem] p-10 text-gray-900 relative z-10">
          <form onSubmit={handleFormSubmit} className="space-y-8">
            <input type="text" required className="w-full bg-gray-50 border-2 border-gray-50 rounded-2xl px-6 py-5 focus:border-indigo-600 outline-none font-bold" placeholder="Full Name" />
            <input type="email" required className="w-full bg-gray-50 border-2 border-gray-50 rounded-2xl px-6 py-5 focus:border-indigo-600 outline-none font-bold" placeholder="Email Address" />
            <textarea rows={4} required className="w-full bg-gray-50 border-2 border-gray-50 rounded-2xl px-6 py-5 focus:border-indigo-600 outline-none font-bold" placeholder="Your Message"></textarea>
            <button className="w-full py-6 bg-indigo-600 text-white rounded-[1.5rem] font-black text-xl">Send Message</button>
          </form>
        </div>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[#FDFDFD]">
      <nav className="fixed top-0 left-0 right-0 h-24 bg-white/80 backdrop-blur-2xl z-[60] border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-8 h-full flex items-center justify-between">
          <button onClick={() => navigate('home')} className="text-3xl font-black text-gray-900 tracking-tighter">
            <span className="text-indigo-600">O</span>F.
          </button>
          <div className="hidden md:flex items-center gap-12">
            {(['about', 'skills', 'projects', 'contact'] as Page[]).map(page => (
              <button key={page} onClick={() => navigate(page)} className={`text-[11px] font-black uppercase tracking-widest ${currentPage === page ? 'text-indigo-600' : 'text-gray-400'}`}>{page}</button>
            ))}
          </div>
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-white p-8 flex flex-col gap-6 shadow-xl border-b border-gray-100">
            {(['about', 'skills', 'projects', 'contact'] as Page[]).map(page => (
      ...
      {selectedProject && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-gray-950/90 backdrop-blur-xl">
          <div className="relative bg-white rounded-[3rem] w-full max-w-4xl p-12 overflow-y-auto max-h-[90vh]">
            <button onClick={() => setSelectedProject(null)} className="absolute top-8 right-8 text-3xl"><i className="fas fa-times"></i></button>
            <h3 className="text-5xl font-black mb-8 capitalize">{selectedProject.name}</h3>
            <p className="text-2xl text-gray-600 mb-10 leading-relaxed">{selectedProject.description}</p>
            <div className="flex gap-4">
              <a href={selectedProject.repoUrl} target="_blank" className="bg-gray-900 text-white px-10 py-5 rounded-2xl font-black">GitHub</a>
              <button onClick={() => handleShare(selectedProject)} className="border-2 border-gray-100 px-10 py-5 rounded-2xl font-black">{isCopied ? 'Copied!' : 'Share'}</button>
            </div>
          </div>
        </div>
      )}
